          ...
          (for [gem grouped-edge-means]
            (do
             (let [[pts (transpose gem)]
                   ...
                   [xres [(min x) (max x)]]]
               (try (do
                     (model_ransac.fit (reshape x [(len x) 1]) (reshape y [(len y) 1]))

                     (figure)

                     (title (+ "RANSAC of " filebasename ", Component " (str i)))
                     (ylabel "Y")
                     (xlabel "X")
                     (plot x y "ro")
                     (plot xres (.predict model_ransac (reshape xres [(len xres) 1])) "b-")

                     (savefig (+ filebasename "-ransac(comp" (str i) ").png")))))
             (setv i (inc i))))))))
